...
本次改动为 Qwen2 推理加入了基于 MPI 的 CPU 张量并行：

- 每个 MPI rank 负责一份权重分片。
- `q/k/v/gate/up` 使用按输出维切分（column parallel）。
- `o/down` 使用按输入维切分（row parallel），局部结果通过 `MPI_Allreduce` 求和。
- KV Cache 按 `num_key_value_heads` 分片，各 rank 只保存本地 KV。
- `embedding / final norm / lm_head` 目前保持复制，以简化实现并保证 logits 一致。
...
```bash
mpirun -np 2 python test/test_infer.py --device cpu --model /path/to/model --test
```

### 约束

当前实现要求以下维度能被 MPI world size 整除：

- `num_attention_heads`
- `num_key_value_heads`
- `intermediate_size`

这是为了保持 GQA 分组和 MLP 分片逻辑简单、稳定。
